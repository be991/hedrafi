...
import BlinkNotification from "./BlinkNotification";
...
const Header = () => {
  ...
  return (
    <header
      className={`fixed top-0 z-[100] w-full transition-all duration-300 px-4 pt-4 ${scrolled ? "translate-y-0" : "translate-y-0"}`}
    >
      <div
        className={`max-w-7xl mx-auto backdrop-blur-xl rounded-[2rem] px-6 py-3 flex justify-between items-center transition-all duration-500 border-white/10 ${
          scrolled
            ? "bg-white/5 shadow-[0_0_50px_rgba(0,0,0,0.3)] scale-[0.98]"
        ...
        <nav className="hidden md:flex items-center gap-1">
          {navLinks.map((link) => (
            ...
            >
              <link.icon
                size={16}
                className={`transition-transform duration-300 group-hover:scale-110 ${isActive(link.path) ? "text-cyber-blue" : "text-gray-600 group-hover:text-gray-400"}`}
              />
              {link.name}
              {isActive(link.path) && (
      ...
      <div
        className={`md:hidden absolute top-full left-4 right-4 mt-4 transition-all duration-500 transform ${
          isMenuOpen
          ...
          {navLinks.map((link) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className={`p-2.5 rounded-xl ${isActive(link.path) ? "bg-blue-600 text-white" : "bg-white/5 text-gray-400"}`}
                >
                  <link.icon size={20} />
                </div>
                <span
                  className={`font-black uppercase tracking-widest text-sm ${isActive(link.path) ? "text-white" : "text-gray-400"}`}
                >
                  {link.name}
                </span>
              </div>
